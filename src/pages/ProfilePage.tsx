              ...
              {badges.map((badge) => (
                ...
                >
                  {badge.rarity && badge.unlocked && (
                    <div
                      className={`absolute top-0 right-0 translate-x-2 -translate-y-2 ${badge.rarityColor} text-white text-[8px] sm:text-[10px] px-1.5 sm:px-2 py-0.5 rounded-full font-black`}
                    >
                      {badge.rarity}
                    </div>
                  )}
                  <div
                    className={`w-12 h-12 sm:w-16 sm:h-16 ${badge.color} rounded-full mx-auto flex items-center justify-center mb-3 sm:mb-4 group-hover:scale-110 transition-transform`}
                  >
                    <span
